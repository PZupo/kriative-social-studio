...
export default function KriativeHeader({ appName }: { appName: string }) {
  ...
  const [currentLang, setCurrentLang] = useState(languages[1]); // Default: PT-BR
    ...
    <header className="bg-white dark:bg-[#1A1A1A] border-b border-[#E5E5E5] dark:border-[#404040] sticky top-0 z-50 shadow-sm">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16 items-center">
          ...
          {/* Seletor de Idioma + Perfil */}
          <div className="flex items-center space-x-4">
            {/* Dropdown de Idioma */}
            <div className="relative">
              <button
                onClick={() => setLangOpen(!langOpen)}
                className="flex items-center space-x-1 px-3 py-1.5 rounded-lg bg-gray-100 dark:bg-[#2A2A2A] hover:bg-gray-200 dark:hover:bg-[#333333] transition text-sm font-medium"
              >
                <span>{currentLang.flag}</span>
                <span className="text-[#333333] dark:text-white">{currentLang.label}</span>
                <svg className="w-4 h-4 text-[#333333] dark:text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                </svg>
              </button>

              {langOpen && (
                <div className="absolute right-0 mt-2 w-40 bg-white dark:bg-[#2A2A2A] rounded-lg shadow-lg border border-[#E5E5E5] dark:border-[#404040] z-50">
                  {languages.map((lang) => (
                    <button
                      key={lang.code}
                      onClick={() => {
                        setCurrentLang(lang);
                        setLangOpen(false);
                        // Aqui você pode adicionar lógica de tradução no futuro
                    ...
                    >
                      <span>{lang.flag}</span>
                      <span className="text-[#333333] dark:text-white">{lang.label}</span>
                    </button>
                  ))}
                </div>
              )}
      ...
      {open && (
        <div className="md:hidden bg-white dark:bg-[#1A1A1A] border-t dark:border-[#404040]">
          <div className="px-4 py-3 space-y-2">
            ...
            <div className="border-t dark:border-[#404040] pt-2">
              <p className="text-sm font-medium text-[#333333] dark:text-white mb-1">Idioma</p>
              {languages.map((lang) => (
                <button
                  key={lang.code}
                  onClick={() => {
                  ...
                  className="w-full text-left px-3 py-2 text-sm hover:bg-gray-100 dark:hover:bg-[#333333] flex items-center space-x-2"
                >
                  <span>{lang.flag}</span>
